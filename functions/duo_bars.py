...
def viz(df, class_avg, name, save_dir, max_len = 15):
    ...
    plt.title(f"{name}")
    plt.barh(data["team"], data["avg"], color = colors[0], label = "Class Average")
    plt.barh(data["team"], data["avg"] - class_avg, color = colors[1], label = "Team Average")
    plt.barh(data["team"], data["avg"] + class_avg, color = colors[2], label = "Class Average")
    plt.legend()
    plt.xlabel("Average")
    plt.ylabel("Team")
    ...
    bar_loc = [((i + 1) * space) + (i * 2 * each_col_width) + each_col_width for i in range(n)]
    ...
    ax.bar(height = class_avg, x=x_right, color = colors[1], align='center', width=each_col_width)
